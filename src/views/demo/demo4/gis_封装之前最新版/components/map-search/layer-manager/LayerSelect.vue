  ...
  <section class="layer_select_wrap">
    <div class="Default_layer_wrap layer_select_cont">
      ...
      <el-checkbox-group v-model="defaultLayerSelect">
        <el-checkbox v-for="(item, index) in defaultLayerList" :key="index" :label="item.layerName"
          @change="selectDefaultLayer">{{ item.layerName }}</el-checkbox>
      </el-checkbox-group>
    </div>
    <div class="custom_layer_wrap layer_select_cont">
      <h3>自定义</h3>
      <el-radio-group v-model="customLayerSelect">
        <el-radio v-for="(item, index) in customLayerList" :key="index" :label="item.id" @change="selectCustomLayer">
          {{ item.layerName }}
        </el-radio>
      </el-radio-group>
    </div>
...
const initLayerData = async () => {
  try {
    ...
    defaultLayerSelect.value = [defaultLayerList.value[0].layerName, defaultLayerList.value[1].layerName]  // 默认[多选]
  } catch (e) {
    console.warn(e)
  }
}
