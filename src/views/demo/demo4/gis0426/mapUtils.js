 ...
 */
const popupCommonConfig = (olMap, pixelPoint, popupData, next, overlayConfig = null,) => {
...
export const addCurve = (olMap, curveDataList) => {
  ...
  let strokeColor = 'rgba(255, 205, 67, 0.3)'
  ...
  curveDataList.forEach(item => {
    ...
    switch (item.curveData.workFrequency) {
      case 'A频段':
        curveStyle = setFeaturesStyle('rgba(200, 22, 100, 0.4)', strokeColor)
        curveRadius = 100
        break;
      case 'D频段':
        curveStyle = setFeaturesStyle('rgba(55, 33, 188, 0.4)', strokeColor)
        curveRadius = 110
        break;
      case 'E频段':
        curveStyle = setFeaturesStyle('rgba(88, 99, 200, 0.4)', strokeColor)
        curveRadius = 120
        break;
      case 'F频段':
        curveStyle = setFeaturesStyle('rgba(255, 255, 0, 0.4)', strokeColor)
        curveRadius = 130
        break;
      case 'FDD':
        curveStyle = setFeaturesStyle('rgba(255, 76, 127, 0.4)', strokeColor)
        curveRadius = 140
        break;
      default:
        curveStyle = setFeaturesStyle('rgba(32, 222, 230, 0.4)', strokeColor)
        curveRadius = 150
        break
    }
 ...
 * @param {*} next 事件处理方法
 */
export const setPopup = (olMap, e, popupData, next) => {
...
// 自动弹出气泡窗
export const setAutoPopup = (olMap, pixelPoint, itemData) => {
  // 点击尺 （这里是尺(米)，并不是经纬度）;
  ...
  popupCommonConfig(olMap, pixelPoint, popupInner.commonPopupInner(popupObj), e => {
    const { target } = e  // 事件对象
